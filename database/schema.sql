...
-- Weekly Coach Summaries — one row per completed week (Monday-indexed)
-- Generated by ml/weekly_summary.py on Mondays via the daily pipeline.
-- narrative is Claude-generated coach copy; falls back to a template if
-- ANTHROPIC_API_KEY is absent or the API call fails.
CREATE TABLE IF NOT EXISTS weekly_summaries (
    id               SERIAL PRIMARY KEY,
    week_start       DATE          NOT NULL UNIQUE,
    map_avg_this     NUMERIC(6, 2),
    map_avg_last     NUMERIC(6, 2),
    map_delta        NUMERIC(6, 2),
    readings_this    INTEGER,
    top_driver       TEXT,
